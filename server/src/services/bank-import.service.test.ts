...
test("Monzo CSV parser preserves quoted fields, stable IDs, signs and provenance",()=>{
  ...
  assert.match(rows[1].sourceRecordId,/^csv_[0-9a-f]{64}_[0-9a-f]{64}_1$/);assert.equal(rows[1].direction,"credit");assert.equal(rows[1].occurredAt,null,"timezone-less statement times must not be invented as UTC");
});
...
test("legitimate identical rows without provider IDs retain separate file-row identities",()=>{const rows=parseMonzoCsv(Buffer.from("Date,Amount,Currency\n18/08/2026,-1,GBP\n18/08/2026,-1,GBP\n"));assert.equal(rows.length,2);assert.notEqual(rows[0].sourceRecordId,rows[1].sourceRecordId);});
