...
export const auditVatFilingEvidence=async(eventType:"vat_filing_evidence_linked"|"vat_filing_evidence_unlinked",filingId:string,evidenceId:string,purpose:string,userId:string,db:Db=pool)=>{
  ...
  await db.query(`INSERT INTO finance_os.finance_events(event_type,entity_type,entity_id,summary,metadata,created_by) VALUES($1,'vat_filing',$2::uuid,$3,jsonb_build_object('vat_filing_id',$2::uuid::text,'vat_period_id',$4::uuid::text,'obligation_reference',$5::text,'evidence_id',$6::uuid::text,'evidence_purpose',$7::text,'actor',$8::uuid::text,'timestamp',now()),$8::uuid)`,[eventType,filing.id,eventType==="vat_filing_evidence_linked"?"VAT filing evidence linked":"VAT filing evidence unlinked",filing.vat_period_id,filing.obligation_reference,evidenceId,purpose,userId]);
};
