...
export const listVatFilings=async(db:Db=pool)=>(await db.query(`SELECT id,vat_period_id,obligation_reference,submitted_at,submission_method,result,box_1,box_2,box_3,box_4,box_5,box_6,box_7,box_8,box_9,source_ledger_fingerprint,notes,created_at,created_by,true AS hmrc_receipt_recorded FROM finance_os.vat_filings ORDER BY submitted_at DESC`)).rows;
