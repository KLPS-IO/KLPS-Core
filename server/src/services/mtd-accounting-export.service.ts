...
export const MTD_VALIDATION_CONTRACT_VERSION=4 as const;
...
export type ExportBlockerDetail=
  ...
  |{code:"vat_period_date_conflict";expense_id:string;stored_vat_period_id:string;effective_tax_point_date:string;date_derived_vat_period_id:string|null;date_derived_vat_period_source:"derived"|"none"|"conflict";date_derived_matching_period_ids:string[];message:string}
...
export const validateExpenseForQuickFile=(row:Json,config:ExportConfig):{disposition:"eligible"|"blocked"|"excluded";reasons:string[];row:ExportRow|null;nominal:string;paymentCode:string;canonicalWarnings:CanonicalWarning[];blockerDetails:ExportBlockerDetail[]}=>{
  ...
  const periodConflict=row.vat_period_date_conflict as Json|undefined;
  if(periodConflict){
    reasons.push("vat_period_date_conflict");
    blockerDetails.push({code:"vat_period_date_conflict",expense_id:clean(row.id),stored_vat_period_id:clean(periodConflict.stored_vat_period_id),effective_tax_point_date:dateOnly(periodConflict.effective_tax_point_date),date_derived_vat_period_id:nullableText(periodConflict.date_derived_vat_period_id),date_derived_vat_period_source:(clean(periodConflict.date_derived_vat_period_source)||"none") as "derived"|"none"|"conflict",date_derived_matching_period_ids:Array.isArray(periodConflict.date_derived_matching_period_ids)?periodConflict.date_derived_matching_period_ids.map(String):[],message:"The stored VAT period does not contain the effective tax-point date. Founder review is required before accounting export."});
  }
...
export async function validateAccountingExport(input:Json,config?:ExportConfig,db:Db=pool):Promise<ExportValidation>{
  ...
  const rows:ExportRow[]=[],blockingReasons:Record<string,string[]>={},exclusionReasons:Record<string,string[]>={},blockingDetails:Record<string,ExportBlockerDetail[]>={},canonicalWarnings:Record<string,CanonicalWarning[]>={},mapped:Record<string,string>={},paymentMapped:Record<string,string>={},missing=new Set<string>(),dispositions:Array<{expense_id:string;disposition:"eligible"|"blocked"|"excluded";reasons:string[]}>=[];
  const missingPayment=new Set<string>();
  for(const expense of ordered){const result=validateExpenseForQuickFile(expense,resolved.config);const expenseId=clean(expense.id);dispositions.push({expense_id:expenseId,disposition:result.disposition,reasons:result.reasons});canonicalWarnings[expenseId]=result.canonicalWarnings;if(result.disposition==="excluded"){exclusionReasons[expenseId]=result.reasons;continue;}if(result.blockerDetails.length)blockingDetails[expenseId]=result.blockerDetails;if(result.nominal)mapped[clean(expense.category)||"Uncategorised"]=result.nominal;else missing.add(clean(expense.category)||"Uncategorised");const source=resolvePaymentSource(expense);if(result.paymentCode)paymentMapped[source]=result.paymentCode;else if(dateOnly(expense.payment_date))missingPayment.add(source);if(result.row)rows.push(result.row);else blockingReasons[expenseId]=result.reasons;}
  const manual=adjustments.map(manualAdjustmentProjection);
  const source={validation_contract_version:MTD_VALIDATION_CONTRACT_VERSION,period_id:periodId,profile:QUICKFILE_PROFILE,selected_expense_ids:ordered.map(row=>clean(row.id)),eligible_expense_ids:rows.map(row=>row.expense_id),dispositions,ledger:ordered.map(row=>({...row,evidence_files:(row.evidence_files as Json[]|undefined)?.map(e=>({id:e.id,type:e.type}))})),adjustments,config:resolved.config,config_source:resolved.source,config_confirmed:resolved.confirmed,config_version:resolved.version};
