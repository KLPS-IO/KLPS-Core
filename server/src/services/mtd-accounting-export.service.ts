...
export const MTD_VALIDATION_CONTRACT_VERSION=2 as const;
...
export const validateExpenseForQuickFile=(row:Json,config:ExportConfig):{reasons:string[];row:ExportRow|null;nominal:string;paymentCode:string;canonicalWarnings:CanonicalWarning[];blockerDetails:ExportBlockerDetail[]}=>{
  ...
  for(const warning of warnings){
    ...
    else if(warning==="no_supplier_invoice"){
      reasons.push("supplier_document_review_required");
      blockerDetails.push({code:"supplier_document_review_required",expense_id:clean(row.id),message:treatment==="no_vat_shown"?"No VAT is being claimed where the treatment is no VAT shown, but the supporting document still requires founder review before accounting export.":"The supporting supplier document requires founder review before accounting export."});
    }
    else if(warning==="possible_duplicate")reasons.push("possible_duplicate_review_required");
  }
  if(reasons.length)return{reasons:[...new Set(reasons)],row:null,nominal,paymentCode,canonicalWarnings,blockerDetails};
  const exportVat=present(vat)?vat:0,exportRate=present(row.vat_rate)?Number(row.vat_rate):0;
  const rate=exportRate<=1?exportRate*100:exportRate;
  return{reasons:[],nominal,paymentCode,canonicalWarnings,blockerDetails,row:{expense_id:clean(row.id),values:{
...
export async function validateAccountingExport(input:Json,config?:ExportConfig,db:Db=pool):Promise<ExportValidation>{
  ...
  const rows:ExportRow[]=[],blockingReasons:Record<string,string[]>={},blockingDetails:Record<string,ExportBlockerDetail[]>={},canonicalWarnings:Record<string,CanonicalWarning[]>={},mapped:Record<string,string>={},paymentMapped:Record<string,string>={},missing=new Set<string>();
  const missingPayment=new Set<string>();
  for(const expense of ordered){const result=validateExpenseForQuickFile(expense,resolved.config);const expenseId=clean(expense.id);canonicalWarnings[expenseId]=result.canonicalWarnings;if(result.blockerDetails.length)blockingDetails[expenseId]=result.blockerDetails;if(result.nominal)mapped[clean(expense.category)||"Uncategorised"]=result.nominal;else missing.add(clean(expense.category)||"Uncategorised");const source=resolvePaymentSource(expense);if(result.paymentCode)paymentMapped[source]=result.paymentCode;else if(dateOnly(expense.payment_date))missingPayment.add(source);if(result.row)rows.push(result.row);else blockingReasons[expenseId]=result.reasons;}
  ...
  return{export_type:MTD_EXPORT_TYPE,profile:QUICKFILE_PROFILE,validation_mode:"dry_run",generated_at:new Date().toISOString(),vat_period:period,eligible_row_count:rows.length,blocked_row_count:Object.keys(blockingReasons).length,blocked_expense_ids:Object.keys(blockingReasons),blocking_reasons:blockingReasons,blocking_details:blockingDetails,canonical_warnings:canonicalWarnings,mapping_config_source:resolved.source,mapping_config_confirmed:resolved.confirmed,mapping_config_version:resolved.version,mapped_nominal_codes:mapped,missing_nominal_mappings:[...missing].sort(),payment_account_mappings:paymentMapped,unmapped_payment_sources:[...missingPayment].sort(),adjustment_handling:{strategy:"exclude_from_purchase_csv_and_require_manual_credit_note",manual_adjustment_count:manual.length,items:manual},expected_csv_headings:QUICKFILE_HEADERS,source_ledger_fingerprint:fingerprint(source),rows};
}
