...
const parsePayload = (
  ...
  try {
    return JSON.parse(rawPayload) as Record<
      string,
      unknown
    >;
  }
...
const getRecordingMetadata = (
  ...
  for (const candidate of candidates) {
    if (
      ...
      return candidate[index] as Record<
        string,
        unknown
      >;
    }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const submission =
        await submitResearchResponse({
          ...
          frequency:
            ...
            null,
          currentSolutions:
            payload.currentSolutions ?? [],
          ageRange:
            asString(payload.ageRange) ||
            undefined,
          employmentStatus:
            asString(
          ...
          occupation:
            asString(payload.occupation) ||
            undefined,
          lifeStage:
            asString(payload.lifeStage) ||
            undefined,
          incomeBand:
            asString(payload.incomeBand) ||
            undefined,
          challengeFrequency:
            asString(
              payload.challengeFrequency
            ) || undefined,
          confidenceLevel:
            asOptionalNumber(
          ...
          spentMoney:
            asString(payload.spentMoney) ||
            undefined,
          spentMoneyOn:
            asStringArray(
              payload.spentMoneyOn
            ),
          wouldUse:
            asString(payload.wouldUse) ||
            undefined,
          wouldPay:
            asString(payload.wouldPay) ||
            undefined,
          monthlyPrice:
            asString(payload.monthlyPrice) ||
            undefined,
          desiredInsights:
            asStringArray(
              payload.desiredInsights
            ),
          otherInsight:
            asString(payload.otherInsight) ||
            undefined,
          trustedSource:
            asString(
              payload.trustedSource ??
              payload.trusted_source
            ) || undefined,
          recordings:
            uploadedFiles.map(
              uploadedFile =>
      ...
      researchLog(
        "RESEARCH_SUBMISSION_COMPLETE",
        {
          participantId:
            submission.participantId,
          surveyResponseId:
            submission.surveyResponseId,
          recordingCount:
