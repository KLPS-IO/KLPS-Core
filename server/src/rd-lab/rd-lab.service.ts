...
type Kind = "text" | "text_array" | "date" | "timestamp" | "boolean" | "money" | "ratio" | "uuid" | "email" | "url";
...
export const RD_RESOURCES = {
  ...
  findings: { table:"technical_findings",required:["work_package_id","title","finding","change_reason"],fields:{work_package_id:"uuid",supplier_id:"uuid",interaction_id:"uuid",title:"text",finding:"text",source_type:"text",impact_on_mvp:"text",decision_required:"boolean",confidence:"ratio",verification_status:"text",founder_review_required:"boolean",status:"text",...baseAudit},enums:{status:['Observation','To Validate','Accepted','Rejected','Superseded'],verification_status:['Unknown','Unverified','Under Review','Verified','Rejected','Expired']} },
  actions: { table:"action_items",required:["work_package_id","title","owner","priority","change_reason"],fields:{work_package_id:"uuid",supplier_id:"uuid",interaction_id:"uuid",title:"text",description:"text",owner:"text",priority:"text",due_date:"date",status:"text",completed_at:"timestamp",...baseAudit},enums:{priority:['Critical','High','Medium','Low'],status:actionStatuses} },
...
function parse(field:string,kind:Kind,value:unknown) {
  ...
  const amount=toFiniteMoney(value);if(amount===null)throw fail(`${field} must be finite money or null`);
  if(kind==="ratio"&&(amount<0||amount>1))throw fail(`${field} must be between 0 and 1`);
  return amount;
}
...
export async function listRecords(resource:RdResource,wpId:string,query:Record<string,unknown>,db:Db=pool){
  ...
  if(resource==="interactions") return (await db.query(`SELECT i.*,
    COALESCE((SELECT jsonb_agg(jsonb_build_object('id',e.id,'title',e.title,'relationship',l.relationship) ORDER BY l.display_order,l.created_at) FROM finance_os.evidence_links l JOIN finance_os.evidence e ON e.id=l.evidence_id WHERE l.entity_type='rd_interaction' AND l.entity_id=i.id AND l.hidden=false),'[]'::jsonb) linked_evidence,
    COALESCE((SELECT jsonb_agg(jsonb_build_object('id',f.id,'title',f.title,'status',f.status) ORDER BY f.created_at) FROM rd_lab.technical_findings f WHERE f.interaction_id=i.id),'[]'::jsonb) linked_findings,
    COALESCE((SELECT jsonb_agg(jsonb_build_object('id',a.id,'title',a.title,'status',a.status) ORDER BY a.created_at) FROM rd_lab.action_items a WHERE a.interaction_id=i.id),'[]'::jsonb) linked_actions,
    COALESCE((SELECT jsonb_agg(jsonb_build_object('id',d.id,'title',d.title,'status',d.status) ORDER BY d.created_at) FROM finance_os.decisions d WHERE d.interaction_id=i.id),'[]'::jsonb) linked_decisions,
    COALESCE((SELECT jsonb_agg(jsonb_build_object('id',r.id,'title',r.title,'status',r.status) ORDER BY r.created_at) FROM finance_os.risks r WHERE r.interaction_id=i.id),'[]'::jsonb) linked_risks
    FROM rd_lab.interactions i WHERE ${where.replace(/\bwork_package_id\b/g,"i.work_package_id")} ORDER BY i.occurred_at DESC NULLS LAST,i.created_at DESC LIMIT 500`,params)).rows;
